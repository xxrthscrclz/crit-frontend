...
const TestAccountSelectModal = ({ onClose, onLogin }: TestAccountSelectModalProps) => {
  ...
  return createPortal(
    ...
    >
      <div
        className="flex flex-col w-200 px-8 py-8 gap-6 bg-white rounded-xl border-2 border-accent shadow-[0_4px_20px_rgba(130,87,180,0.2)] animate-modal-in max-md:mx-4 max-md:w-full max-md:max-w-200 max-md:px-5 max-md:py-6 max-md:gap-5"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex w-full justify-between items-center">
          <div className="text-black typo-title2">테스트 계정 선택</div>
          <CloseIcon className="crit-icon-brand w-6 h-6 cursor-pointer" onClick={onClose} />
        </div>
            ...
            return (
              <button
                ...
                className={`flex flex-1 flex-col items-center justify-center gap-2 min-h-28 px-4 py-4 rounded-xl border-2 text-center cursor-pointer transition-colors ${
                  isSelected
                    ? 'border-brand-strong bg-accent-soft'
                    : 'border-neutral bg-white hover:border-accent-muted'
                }`}
              >
                <div className="text-black typo-body1-medium">{account.channelName}</div>
                <div className="text-muted typo-body2">{account.category}</div>
              </button>
            );
          })}
        </div>
        ...
        <button
          ...
          className="flex py-2.5 px-5 mx-auto justify-center items-center rounded-lg bg-brand-strong active:bg-brand-deep typo-body1-medium text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          로그인
        </button>
      </div>
