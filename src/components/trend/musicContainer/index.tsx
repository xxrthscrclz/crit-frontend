...
const MusicContainer = ({ rank, item }: MusicContainerProps) => {
  ...
  return (
    <div className="group flex w-full min-w-0 shrink-0 items-center justify-between gap-3 rounded-xl px-3 py-2.5 transition-colors duration-200 hover:bg-brand-soft/6 max-md:gap-2 max-md:px-2 max-md:py-2">
      <div className="flex min-w-0 flex-1 items-center gap-3 max-md:gap-2">
        ...
        <div className="flex min-w-0 flex-col justify-center gap-0.5">
          ...
          <div className="truncate text-muted typo-body6">{item.artist}</div>
        </div>
      </div>
      <div className="flex shrink-0 items-center gap-2 opacity-80 transition-opacity duration-200 group-hover:opacity-100 max-md:gap-1">
        <button
          type="button"
          onClick={handlePlay}
          className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-muted transition-colors hover:bg-brand-soft/10 hover:text-brand active:text-brand max-md:h-7 max-md:w-7"
        ...
        <button
          type="button"
          onClick={handleShare}
          className={`flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg transition-colors hover:bg-brand-soft/10 max-md:h-7 max-md:w-7 ${
            shared ? 'text-brand' : 'text-muted hover:text-brand active:text-brand'
          }`}
