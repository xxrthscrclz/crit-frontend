...
const VideoTitle = ({ title, isRegenerating = false, onRegenerate }: VideoTitleProps) => {
  ...
  return (
    <div className="flex items-center justify-end gap-10 self-stretch rounded-xl border border-brand-secondary bg-surface-raised px-4 py-3 max-md:flex-col max-md:gap-3 max-md:px-3">
      <div
      ...
      <div className="flex h-12 w-17 flex-col justify-end max-md:h-auto max-md:w-auto max-md:flex-row max-md:gap-4">
        <div
          onClick={handleCopy}
          className={`flex justify-end items-center gap-1.5 cursor-pointer ${copied ? 'text-brand' : 'text-placeholder active:text-brand'}`}
        >
        ...
        <div
          ...
          className={`flex justify-end items-center gap-1.5 ${isRegenerating || !onRegenerate ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} ${isRegenerating ? 'text-brand' : 'text-placeholder active:text-brand'}`}
        >
