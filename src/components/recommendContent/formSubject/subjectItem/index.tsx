  ...
  return (
    <div
      ...
      className={`flex w-234 cursor-pointer flex-col items-start gap-3 rounded-xl border px-8 py-6 transition-all duration-200 max-md:w-full max-md:gap-2 max-md:px-4 max-md:py-4
        ${
          selected
            ? 'border-brand bg-input-highlight shadow-md'
            : 'border-brand-secondary bg-white hover:bg-input-highlight'
        }`}
    >
      {subject ? (
        <div className="typo-title-bold text-primary">{subject}</div>
      ) : (
      ...
      {subjectContent && (
        <div className="typo-body2 text-muted whitespace-pre-line">{subjectContent}</div>
      )}
    </div>
  );
};
